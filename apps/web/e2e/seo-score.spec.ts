...
test.describe("Solo Compass — SEO 100 rubric", () => {
  test("both locales", async ({ page, baseURL }) => {
    ...
    for (const { locale, path, expectHtmlLang } of LOCALES) {
      ...
      const [descMin, descMax, descSoftMin, descSoftMax] =
        locale === "zh" ? [70, 100, 55, 130] : [120, 160, 80, 200];
      ...
      record(
        "description",
        `Meta description length (${descMin}–${descMax} for ${locale})`,
        5,
        descScore,
        `len=${descLen}`,
      );
      ...
      record(
        "hierarchy",
        "Heading hierarchy has no forward jumps",
        5,
        hierarchyOk ? 5 : 2,
      ...
      record(
        "twitter",
        "Twitter Card (card/title/desc/image)",
        4,
        twScore,
      ...
      record(
        "jsonld",
        "JSON-LD SoftwareApp + Org parseable",
        8,
        jsonldScore,
      ...
      record(
        "robots",
        "robots.txt 200 + Sitemap: + User-Agent:",
        4,
        robotsScore,
      ...
      const imgScore = imgs.length === 0 ? 6 : Math.max(0, 6 - missingAlt * 2 - emptyAltNonDecor);
      record(
        "img-alt",
      ...
      const semScore =
        (hasMain ? 2 : 0) + (hasNav ? 1 : 0) + (hasFooter ? 1 : 0) + (hasSection ? 1 : 0);
      record(
        "semantics",
      ...
      const internalHrefs = Array.from(
        new Set(hrefs.filter((h) => h.startsWith("/") && !h.startsWith("//"))),
      );
      const anchorHrefs = Array.from(
        new Set(hrefs.filter((h) => h.startsWith("#") && h.length > 1)),
      );
      ...
      record(
        "links",
        "No broken internal links or dead anchors",
        6,
        linkScore,
      ...
      const perfScore =
        (hasViewport ? 3 : 0) + (lcpOk ? 3 : lcp < 4000 ? 1 : 0) + (noConsole ? 2 : 0);
      record(
        "perf",
        "Viewport + LCP<2500ms + no console errors",
        8,
        perfScore,
      ...
      record(
        "html-lang",
        `<html lang> matches ${expectHtmlLang}`,
        4,
        langOk ? 4 : 1,
      ...
      record(
        "icons",
        "icon + apple-touch-icon (200) + theme-color",
        4,
        iconScore,
