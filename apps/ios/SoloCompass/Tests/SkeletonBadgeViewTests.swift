...
/// and on the boolean gate that decides whether the card shows the badge.
final class SkeletonBadgeViewTests: XCTestCase {
    ...
    /// The card renders the badge only for `.skeleton`; `.real` and `.cached`
    ...
    func testBadgeShownOnlyForSkeletonQuality() {
        ...
        XCTAssertTrue(shouldRender(.skeleton), "skeleton → badge visible")
        XCTAssertFalse(shouldRender(.real), "real → badge hidden")
        XCTAssertFalse(shouldRender(.cached), "cached → badge hidden")
    }

    /// A freshly constructed AIService defaults to `.real`, so a card bound to
    /// it shows no badge until a synthesis actually degrades.
