...
- [ ] `VerifiedMark`(默认 `card` 样式):"已验证路线" / "社群观察中" + 头像堆叠 + "与 N 位旅人走过"
...
#### US-104:VerifiedMark 三种表达(默认标识卡)

**描述**:作为用户,我在路线详情中看到 walked-by 信号,有&quot;已验证路线&quot;或&quot;社群观察中&quot;的清晰区分。

**Acceptance Criteria:**

- [ ] `VerifiedMark` 视图,`style: VerifiedStyle = .card`(默认),另外两档 `.header` `.inline` 作为后续视觉实验
- [ ] `.card`:hero 下方独立卡(白底 / 浅色边),左侧 24pt 图标(已验证 `check-circle`,观察中 `users`)+ 右侧头像堆叠最多 5 个 + "与 N 位旅人走过"
...
- **FR-19**:`VerifiedMark` 默认 `.card` 样式;`.header`/`.inline` 保留代码路径但 UI 中不默认使用。
