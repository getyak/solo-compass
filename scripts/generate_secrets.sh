...
emit_swift() {
    local key="$1" base="$2" model="$3" sb_url="$4" sb_key="$5" sentry="$6" fsq="$7" owm="${8:-}" amap="${9:-}" api_base="${10:-}"
...
enum Secrets {
    ...
    static let apiBaseURL: String = "${api_base}"
}
SWIFT
}
...
SOLO_API_BASE_URL=$(read_env_value "SOLO_API_BASE_URL")
...
emit_swift "${DEEPSEEK_API_KEY}" "${DEEPSEEK_BASE_URL}" "${DEEPSEEK_MODEL}" "${SUPABASE_URL}" "${SUPABASE_ANON_KEY}" "${SENTRY_DSN}" "${FOURSQUARE_API_KEY}" "${OPENWEATHER_API_KEY}" "${AMAP_API_KEY}" "${SOLO_API_BASE_URL}"
